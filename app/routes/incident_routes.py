...
@router.post("/")
def create_incident(
    data: IncidentCreate, current_user: Users = Depends(get_current_user)
):
...
def update_incident(
    ...
    return incident_controller.update_incident_controller(
        incident_id, data, current_user
    )


@router.post("/updates/")
def add_incident_update(
    data: IncidentUpdateEntry, current_user: Users = Depends(get_current_user)
):
